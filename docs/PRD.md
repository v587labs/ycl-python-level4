...
当前课程分为三层：课时 0 为序章，课时 1-12 为 YCL Python 四级主线课，课时 13-28 为拓展挑战课。拓展挑战用于项目练习和能力拔高，不作为四级必学范围。

```mermaid
flowchart TB
    Intro["课时0：Python是什么？"]
    subgraph Chapter1["第1章 Python语言基本语法"]
        L1["课时1-2：变量与常量、数据类型、运算符、注释"]
    end
    subgraph Chapter2["第2章 Python基本数据类型"]
        L2["课时3-4：整数、浮点数、字符串、布尔类型"]
    end
    subgraph Chapter3["第3章 Python程序结构"]
        L3["课时5-6：顺序结构、选择结构if/elif/else"]
        L4["课时7-8：循环结构for/while"]
    end
    subgraph Chapter4["第4章 函数与模块"]
        L5["课时9-10：函数定义、参数传递、作用域、常用模块"]
    end
    ...
    subgraph Extension["拓展挑战"]
        E1["课时13-28：Turtle项目、循环/函数小游戏、数学挑战、题库改造"]
    end
```
...
| 模块 | 功能 | 优先级 | 说明 |
|------|------|--------|------|
| **首页** | 课时选择器 | P0 | 29课时卡片，区分主线与拓展，点击进入 |
...
| 项目 | 验收条件 | 优先级 |
|------|---------|--------|
| 课程覆盖 | 课时 0-12 完整覆盖四级主线，课时 13-28 标记为拓展挑战 | P0 |
